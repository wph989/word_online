"""
HTML 渲染服务（重构版）
使用 Pydantic 模型，将 Content 和 StyleSheet 转换回 HTML
"""
...
class HtmlRenderer:
    """
    HTML 渲染器
    将结构化的 Content 和 StyleSheet 渲染为 HTML
    """
    ...
    def render(self) -> str:
        ...
        # 对blocks进行分组处理，将连续的列表项合并
        i = 0
        while i < len(self.content.blocks):
            block = self.content.blocks[i]
            
            # 检查是否是列表项
            if isinstance(block, ParagraphBlock) and block.attrs and block.attrs.listType:
                # 收集连续的同类型同层级列表项
                list_items = [block]
    ...
    def _render_paragraph(self, block: ParagraphBlock) -> str:
        """
        渲染段落（仅处理普通段落，列表项由 _render_list_group 处理）
    ...
    def _render_table(self, block: TableBlock) -> str:
        ...
        # 使用专门的表格渲染器
        renderer = TableRenderer(block.data, styles)
    ...
    def _apply_marks(self, text: str, marks: List[Mark]) -> str:
        ...
        应用格式标记到文本
        处理重叠标记，避免文本重复
        优化：合并同一位置的多个 ValueMark 到单个 span
        ...
        for i in range(len(sorted_boundaries) - 1):
            ...
            # 4. 分组标记：SimpleMark, LinkMark, ValueMark
